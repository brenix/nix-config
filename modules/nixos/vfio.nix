...
in
{
  options.virtualisation.vfio = {
    ...
    isolateCPUs = mkOption {
      type = types.listOf (types.strMatching "[0-9]+-[0-9]+");
      default = [ ];
      example = [ "8-15" "24-31" ];
      description = "Isolate CPUs from host";
    };
    devices = mkOption {
  ...
  config = lib.mkIf cfg.enable {
    ...
    boot = {
      kernelParams =
        ...
        ++ (optional (builtins.length cfg.isolateCPUs > 0)
          ("nohz_full=" + builtins.concatStringsSep "," cfg.isolateCPUs)
          ("rcu_nocbs=" + builtins.concatStringsSep "," cfg.isolateCPUs)
          ("rcu_nocb_poll"))
