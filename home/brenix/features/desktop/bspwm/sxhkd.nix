{ pkgs, config, ... }: {
  ...
  services.sxhkd = {
    ...
    keybindings = {
      ...
      # spotify
      "super + s" = if config.dpi > 150 then "spotify --force-device-scale-factor=2" else "spotify";

      # slack
      "super + shift + s" = if config.dpi > 150 then "slack --force-device-scale-factor=2" else "slack";
